...
public class LevelPackSaveData implements Closeable
{
    ...
    public LevelPackSaveData(LevelPack pack) throws IOException
    {
        ...
        arf = new AsyncRandomFile(dir.resolve(id + ".mtsv"));
    ...
    public void updateData()
    {
        try
        {
            ...
            UUID packID = pack.getLevelPackID();
            dos.writeLong(packID.getMostSignificantBits());
            dos.writeLong(packID.getLeastSignificantBits());
    ...
    /**
     * Checks if any of the level save-data objects are dirty.
    ...
    private void loadData() throws IOException
    {
        ...
        if (dis.readInt() != VERSION_1)
            throw new IOException("Unsupported save data version.");

        UUID id = new UUID(dis.readLong(), dis.readLong());
        if (!id.equals(pack.getLevelPackID()))
            throw new IOException("Level pack/ save data mismatch.");
